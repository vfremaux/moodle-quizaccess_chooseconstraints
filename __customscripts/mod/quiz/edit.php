 ...
 *
 * CUSTOMISATION : This script has been customized to allow support 
 * for the special randomconstrained type.
 *
 * @package    mod_quiz
...
$questionbank->display('editq',
    ...
    // CHANGE : Add random constrainted questions
    ...
    $randomform = new quiz_add_randomconstrained_form(new moodle_url('/mod/quiz/accessrule/chooseconstraints/addquestion.php'));
    $randomform->set_data(array(
        'returnurl' => $thispageurl->out_as_local_url(false),
        'cmid' => $cm->id,
    ));
    $formdata = new StdClass;
    $formdata->disabled = $quizhasattempts;
    $randomform->set_data($formdata);
    $randomform->display();
    echo '</div>';
    echo '</div>';
    // /CHANGE
